...
void add_EE(uint aEEid, std::vector<std::pair<std::string, G4double> > &aDzMap, double aAirBefore, double aAirMid) {
  aDzMap.push_back(std::make_pair("Fe_absorber_EE", aAirBefore)); // aAirBefore AIR + 0.3 mm Fe
  ...
  if (aEEid == 11 || aEEid == 12) aDzMap.push_back(std::make_pair("Cu_baseplate", 0.)); //1.2 mm
  aDzMap.push_back(std::make_pair("CuW_baseplate", 0.)); //1.2 mm
  if (aEEid == 13) aDzMap.push_back(std::make_pair("CuW_baseplate_550um", 0.)); // 0.55 mm
  if (aEEid == 14) aDzMap.push_back(std::make_pair("CuW_baseplate_610um", 0.)); // 0.61 mm
  aDzMap.push_back(std::make_pair("Cu_absorber_EE", 0.)); // 6 mm
  if (aEEid == 13) aDzMap.push_back(std::make_pair("CuW_baseplate_610um", 0.)); // 0.61 mm
  if (aEEid == 14) aDzMap.push_back(std::make_pair("CuW_baseplate_710um", 0.)); // 0.71 mm
  aDzMap.push_back(std::make_pair("CuW_baseplate", 0.)); // 1.2 mm
  if (aEEid == 11 || aEEid == 12) aDzMap.push_back(std::make_pair("Cu_baseplate", 0.)); //1.2 mm
  aDzMap.push_back(std::make_pair("Kapton_layer", 0.)); // 0.075 mm
  aDzMap.push_back(std::make_pair("Cu_baseplate_25um", 0.)); //  0 + 0.025mm Cu
...
void add_FH(uint aFHid, std::vector<std::pair<std::string, G4double> > &aDzMap, double aAirBefore, double aAirMid) {
  std::string layout = "";
  if (aFHid < 10) layout = "_DAISY";
  if ( ! (aFHid == 1 || aFHid == 7) ) aDzMap.push_back(std::make_pair("Fe_absorber_FH", aAirBefore)); // aAirBefore AIR + 40 mm Fe
  aDzMap.push_back(std::make_pair("PCB"+layout, aAirMid)); // aAirMid AIR + 1.3 mm
  aDzMap.push_back(std::make_pair("Cu_baseplate_175um"+layout, 0.)); // 0.175mm Cu
  aDzMap.push_back(std::make_pair("Si_wafer"+layout, 0.));  // 0.3 mm
  if (aFHid == 5) {
    aDzMap.push_back(std::make_pair("PCB_thin"+layout, 0)); // 1.2 mm
  }
  if(aFHid != 5) {
    aDzMap.push_back(std::make_pair("Cu_baseplate_25um"+layout, 0.)); // 0.025mm Cu
  ...
  if (aFHid != 5 && aFHid < 9)  aDzMap.push_back(std::make_pair("Cu_baseplate"+layout, 0.)); // 1.2 mm
  if (aFHid == 9 || aFHid == 10)  aDzMap.push_back(std::make_pair("CuW_baseplate"+layout, 0.)); // 1.2 mm  
  if (aFHid != 10) aDzMap.push_back(std::make_pair("Cu_baseplate"+layout, 0.)); // 1.2 mm
  aDzMap.push_back(std::make_pair("Cu_absorber_FH", 0.)); // 6 * mm
  }

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......

void defineConfig22_October2018_1(std::vector<std::pair<std::string, G4double> > &aDzMap, G4double &aViewpoint) {
  aViewpoint = 35 * m;
