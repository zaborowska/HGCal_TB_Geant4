...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  // World = Beam line
  G4Box* solidWorld = new G4Box("World", 0.5 * fMaterials->GetBeamLineXY(), 0.5 * fMaterials->GetBeamLineXY(), 0.5 * fMaterials->GetBeamLineLength());
...
void DetectorConstruction::ConstructHGCal() {
  ...
  for (const auto &item : fElementsMap)
  {
    ...
    auto matName = fMaterials->GetLogicalVolume(itemName)->GetMaterial()->GetName();
    std::size_t found = matName.find("G4_");
    if (found != std::string::npos)
      matName.erase(found, found + 3);
    // special treatment for Si (3 x 0.1 mm)
    if (fMaterials->GetLogicalVolume(itemName)->GetName().find("Si_wafer") != std::string::npos)
    {
      matName = "Si";
    }
    if (item.second != 0)
    ...
    if(matName == "Si" && fMaterials->GetThickness(itemName) == 0.3)
      for(int i=0; i<3;i++)
        myfile << std::fixed << std::setprecision(3) << fMaterials->GetThickness(itemName) / 3 << "," << matName << "\n";
    else
      myfile << std::fixed << std::setprecision(3) << fMaterials->GetThickness(itemName) << "," << matName << "\n";
  }
  myfile.close();
  #endif
...
void DetectorConstruction::ConstructSDandField() {
  ...
  SiliconPixelSD* sensitiveSilicon = new SiliconPixelSD((fMaterials->GetSiPixelLogical()->GetName() + "_sensitive").c_str());
  sdman->AddNewDetector(sensitiveSilicon);
  fMaterials->GetSiPixelLogical()->SetSensitiveDetector(sensitiveSilicon);

  SiPMSD* sensitiveSiPM = new SiPMSD((fMaterials->GetAHCALSiPMlogical()->GetName() + "_sensitive").c_str());
...
void DetectorConstruction::SetStepSizeSilicon(G4double val) {
  ...
  fMaterials->GetSiPixelLogical()->SetUserLimits(new G4UserLimits(0, maxTrackLength));
